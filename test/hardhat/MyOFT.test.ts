...
describe('MyOFT Test', function () {
    ...
    it('should send a token from A address to B address via each OFT', async function () {
        ...
        console.log(ethers.utils.hexZeroPad('0xc458647769f6e9288aAA84F53A652ED7670d3308', 32))
        ...
        const encodedSendParams = abiCoder.encode(
            ['tuple(uint32, bytes32, uint256, uint256, bytes, bytes, bytes)'],
            [[eidB, ethers.utils.zeroPad(ownerB.address, 32), tokensToSend, tokensToSend, options, '0x', '0x']]
        )

        console.log({ encodedSendParams })
        console.log('sendParams: ', JSON.stringify(sendParam))
        console.log([
            eidB,
